...
unlink('Spreads/SP1/*')
for(sp1_sn in 1:19){
  df <- SP1_spread[SP1_spread$HomeTeam == final_doublefixture_sp1div[sp1_sn,1] | SP1_spread$AwayTeam == final_doublefixture_sp1div[sp1_sn,1] ,]
  df2 <- SP1_spread[SP1_spread$HomeTeam == final_doublefixture_sp1div[sp1_sn + 1,1] | SP1_spread$AwayTeam == final_doublefixture_sp1div[sp1_sn + 1,1],]
  temp_analysis <- rbind(df,df2)
  ...
  write.csv(temp_analysis,file.path(path,paste(final_doublefixture_sp1div[sp1_sn,1],final_doublefixture_sp1div[sp1_sn + 1,1],".csv",sep = "_")))
...
unlink('Spreads/F1/*')
for(f1_sn in 1:19){
  df <- F1_spread[F1_spread$HomeTeam == final_doublefixture_f1div[f1_sn,1] | F1_spread$AwayTeam == final_doublefixture_f1div[f1_sn,1] ,]
  df2 <- F1_spread[F1_spread$HomeTeam == final_doublefixture_f1div[f1_sn + 1,1] | F1_spread$AwayTeam == final_doublefixture_f1div[f1_sn + 1,1],]
  temp_analysis <- rbind(df,df2)
  ...
  write.csv(temp_analysis,file.path(path,paste(final_doublefixture_f1div[f1_sn,1],final_doublefixture_f1div[f1_sn + 1,1],".csv",sep = "_")))
